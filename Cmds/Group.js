...
keith({
  ...
  const { q, quoted, quotedMsg, mek, reply, isSuperUser, args } = conText;

  // ✅ Superuser check from context
  if (!isSuperUser) return reply("❌ Owner Only Command!");

  // Extract target group JID from arguments
  ...
  if (!targetGroupJid || !targetGroupJid.endsWith('@g.us')) {
    return reply(
      "❌ Please provide a valid group JID!\n\n" +
      "📌 Usage:\n" +
      "• .togroupstatus2 120363425281814502@g.us <text>\n" +
      "• Reply to an image/video with .togroupstatus2 120363425281814502@g.us <caption>\n" +
      "• Or use without JID in target group"
    );
  }

  // Extract actual content (skip the first argument which is JID)
  const contentArgs = args.slice(1).join(' ');
  const contentText = contentArgs || q;

  if (!contentText && !quotedMsg) {
    return reply(
      "📌 Usage:\n" +
      "• .togroupstatus2 <groupJID> <text>\n" +
      "• Reply to an image/video/audio/document/sticker with .togroupstatus2 <groupJID> <caption>\n" +
