...
// Alternative with size and quality options
keith({
  pattern: "toimg2",
  aliases: ["sticker2video", "animate"],
  description: "Convert animated sticker to video with options",
  category: "Converter",
  filename: __filename
}, async (from, client, conText) => {
  const { quotedMsg, q, mek, reply, keithRandom } = conText;
  ...
  if (!quotedMsg?.stickerMessage) {
    return reply("❌ Quote an animated sticker to convert.");
  }

  const isAnimated = quotedMsg.stickerMessage.isAnimated || quotedMsg.stickerMessage.isAnimatedSticker;
  
  if (!isAnimated) {
    return reply("❌ This is not an animated sticker. Use .toimg for static stickers.");
  }

  // Parse options: .toimg2 fps=20 size=256
  let fps = 15;
  let size = 512;
  let quality = 23;
  
  if (q) {
    const fpsMatch = q.match(/fps[=:](\d+)/i);
    if (fpsMatch) fps = Math.min(parseInt(fpsMatch[1]), 30);
    
    const sizeMatch = q.match(/size[=:](\d+)/i);
    if (sizeMatch) size = Math.min(parseInt(sizeMatch[1]), 1024);
    
    const qualityMatch = q.match(/quality[=:](\d+)/i);
    if (qualityMatch) quality = Math.min(parseInt(qualityMatch[1]), 51);
  }

  try {
    const buffer = await downloadMediaBuffer(client, quotedMsg.stickerMessage, 'sticker');
    const inputPath = keithRandom('.webp');
    const outputPath = keithRandom('.mp4');
    fs.writeFileSync(inputPath, buffer);

    await new Promise((resolve, reject) => {
      exec(`ffmpeg -i ${inputPath} -vf "fps=${fps},scale=${size}:${size}:flags=lanczos,format=yuv420p" -c:v libx264 -preset fast -crf ${quality} -pix_fmt yuv420p -movflags +faststart ${outputPath}`, 
        { timeout: 60000 }, 
        (err) => {
          fs.unlinkSync(inputPath);
          if (err) reject(err);
    ...
    console.error("toimg2 error:", e);
    await reply("❌ Unable to convert animated sticker. " + e.message);
  }
});
//============================
