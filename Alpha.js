...
import keithConnect, { isJidGroup, jidNormalizedUser, isJidBroadcast, downloadMediaMessage, downloadContentFromMessage, DisconnectReason, getContentType, fetchLatestBaileysVersion, useMultiFileAuthState, makeCacheableSignalKeyStore, jidDecode } from '@whiskeysockets/baileys'
import { keithStore, loadSession, keithBuffer, toPtt, keithJson, formatAudio, keithRandom, formatVideo, verifyJidState } from './lib/botFunctions.js'
import { getSudoNumbers, setSudo, delSudo, isSudo } from './database/sudo.js'
import { session, dev, botexpiration } from './settings.js'
import { keith, commands, evt } from './commandHandler.js'
let Sticker, createSticker, StickerTypes;
try {
  ({ Sticker, createSticker, StickerTypes } = await import('wa-sticker-formatter'));
} catch(_stickerErr) {
  console.warn('⚠️  wa-sticker-formatter unavailable (sharp not installed). Sticker commands will be disabled.');
}
import KeithLogger from './logger.js'
...
import { initAntiSpamDB, getAntiSpamSettings, addUserMessage, getUserMessageCount, clearUserMessages, clearAllGroupMessages, getSpamWarnCount, incrementSpamWarnCount, resetSpamWarnCount, clearAllSpamWarns } from './database/antispam.js'
import { initAntiBotDB, getAntiBotSettings, getBotWarnCount, incrementBotWarnCount, resetBotWarnCount, clearAllBotWarns } from './database/antibots.js'
import { initAntiCallDB, getAntiCallSettings, updateAntiCallSettings, getCallCount, incrementCallCount, resetCallCount, getCallWarnCount, incrementCallWarnCount, resetCallWarnCount, clearAllCallWarns } from './database/anticall.js'
import { initAntiBadDB, getAntiBadSettings, getBadWords, containsBadWord, getBadWarnCount, incrementBadWarnCount, resetBadWarnCount, clearAllBadWarns } from './database/antibad.js'
import { initAntiTagDB, getAntiTagSettings, getTagWarnCount, incrementTagWarnCount, resetTagWarnCount, clearAllTagWarns } from './database/antitag.js'
import { initAntiStickerDB, getAntiStickerSettings, getStickerWarnCount, incrementStickerWarnCount, resetStickerWarnCount, clearAllStickerWarns } from './database/antisticker.js'
import { initAutoBlockDB, getAutoBlockSettings, getTriggerWords, containsTriggerWord, getBlockWarnCount, incrementBlockWarnCount, resetBlockWarnCount, clearAllBlockWarns } from './database/autoblock.js'
import { initAntiDeleteDB, getAntiDeleteSettings } from './database/antidelete.js'
import { getGreetSettings, initGreetDB, repliedContacts, updateGreetSettings } from './database/greet.js'
import { initAutoStatusDB, getAutoStatusSettings } from './database/autostatus.js'
import { getAutoReadSettings, initAutoReadDB } from './database/autoread.js'
import { initSettingsDB, getSettings, updateSettings, getSetting } from './database/settings.js'
import { initAutoBioDB, getAutoBioSettings, updateAutoBioSettings } from './database/autobio.js'
import { initAntiLinkDB, getAntiLinkSettings, updateAntiLinkSettings, getWarnCount, incrementWarnCount, resetWarnCount, clearAllWarns } from './database/antilink.js'
import { initAntiStatusMentionDB, getAntiStatusMentionSettings, updateAntiStatusMentionSettings, getStatusWarnCount, incrementStatusWarnCount, resetStatusWarnCount, clearAllStatusWarns } from './database/antistatusmention.js'
import { initPresenceDB, getPresenceSettings } from './database/presence.js'
import { initChatbotDB, saveConversation, getConversationHistory, clearConversationHistory, getLastConversation, getChatbotSettings, updateChatbotSettings, availableVoices } from './database/chatbot.js'
import { initGroupEventsDB, getGroupEventsSettings } from './database/groupevents.js'
...
async function startKeith() {
    try {
        ...
        try {
            ...
            for (const fileName of fs.readdirSync(pluginsPath)) {
                if (path.extname(fileName).toLowerCase() === '.js') {
                    try {
                        await import(pathToFileURL(path.join(pluginsPath, fileName)).href);
                    } catch (e) {
...
async function sendDeletedMessageNotification(client, settings, {
    ...
    if (deletedMsg.message.conversation) {
        ...
        try {
            ...
            if (deletedMsg.message.imageMessage) {
                const buffer = await client.downloadMediaMessage(deletedMsg.message.imageMessage);
                await client.sendMessage(targetJid, {
                    image: { url: buffer },
                ...
                const buffer = await client.downloadMediaMessage(deletedMsg.message.videoMessage);
                await client.sendMessage(targetJid, {
                    video: { url: buffer },
                    caption: notification,
                    mentions: [deleterJid, senderJid],
    ...
    await client.sendMessage(targetJid, {
        ...
        contextInfo
                ...
                const buffer = await client.downloadMediaMessage(deletedMsg.message.audioMessage);
                await client.sendMessage(targetJid, {
                    audio: { url: buffer },
                ...
                const buffer = await client.downloadMediaMessage(deletedMsg.message.stickerMessage);
                await client.sendMessage(targetJid, {
                    sticker: { url: buffer },
                    mentions: [deleterJid, senderJid],
                    contextInfo
    ...
    if (isCommandMessage && cmd) {
        ...
        if (keithCmd) {
            ...
            try {
                ...
                client.downloadMediaMessage = async (message, filename, attachExtension = true) => {
                    try {
                        ...
                        KeithLogger.error("Error in downloadMediaMessage:", error);
                        throw error;
                    }
                };
...
client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "connecting") {
        KeithLogger.info("Alpha md is connecting..");
        reconnectAttempts = 0;
    }
...
if (connection === "open") {
 ...
 const inviteCode = "HuNy0rWnWy43bckQLNXyqL";
...
`));
    ...
    console.log(chalk.magentaBright('✨ ') + 
                chalk.cyanBright('ALPHA MD') + 
                chalk.magentaBright(' ✨') + 
                chalk.blueBright(' is now active and connected!\n'));
    
    KeithLogger.success("✅ Alpha md is active , enjoy 😀");
    reconnectAttempts = 0;
    startAutoBio();
    botExpirationDate(); 
    scheduleMessage(); 
